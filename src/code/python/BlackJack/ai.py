...
class Agent:
    ...
    def make_one_transition(self, action):
        ...
        #The next state of the terminal state is None
        if self.simulator.state == WIN_STATE or self.simulator.state  == LOSE_STATE:
            return None
        a = self.simulator.state
        if action == HIT:
    ...
    def MC_run(self, num_simulation, tester=False):
        ...
        for simulation in range(num_simulation):
            ...
            path = []
            current_S = self.simulator.state
           
            #Getting the simulation sequences and will save the sequences in the varialbe "path"
            while current_S !=  None:
                action = self.default_policy(current_S)
                next_S = self.make_one_transition(action)
                self.N_MC[current_S] = self.N_MC[current_S] + 1
                path.append(current_S)
                current_S = next_S
        
            #determine the value of reward
            for i in range(len(path)):
                sum = self.reward_to_go(path[i:] , DISCOUNT)
                curr = path[i]
                self.S_MC[curr ] = self.S_MC[curr] + sum
                self.MC_values[curr] = self.S_MC[curr] / self.N_MC[curr]
               
    def reward_to_go(self,path,gamma):
        exp = 0
        sum = 0
        for i in range(len(path)):
            if path[i] == LOSE_STATE:
                reward = -1
            elif path[i] == WIN_STATE:
                reward = 1
            else:
                reward = 0
            sum = (gamma ** exp) * reward + sum
    ...
    def TD_run(self, num_simulation, tester=False):
        ...
        for simulation in range(num_simulation):
            ...
            #Randomly, current_S(S_0) can be the terminal state
            while current_S !=  None:
                action = self.default_policy(current_S)
                next_S = self.make_one_transition(action)
                self.N_TD[current_S] = self.N_TD[current_S] + 1
                
                if current_S == WIN_STATE:
                    R = 1
                    U_Next_S = 0
                    U_S = self.TD_values[current_S]
                elif current_S == LOSE_STATE:
                    R = -1
                    U_Next_S = 0
                    U_S = self.TD_values[current_S]
                else:
                    R = 0
                    U_Next_S = self.TD_values[next_S]
                    U_S = self.TD_values[current_S]

                ALPHA = self.alpha(self.N_TD[current_S])
                
                self.TD_values[current_S] = U_S + ALPHA * (R + DISCOUNT * U_Next_S - U_S)
    ...
    def Q_run(self, num_simulation, tester=False, epsilon=0.4):
        ...
        for simulation in range(num_simulation):
            ...
            while current_S !=  None:
                ...
                #Getting the action by pick_action function
                action = self.pick_action(current_S,epsilon)
                next_S = self.make_one_transition(action) 
                self.N_Q[current_S][action] +=  1
                
                #Declare the value needed to be used in the calculation part
                if current_S == WIN_STATE:
                    R_S = 1
                    MaxV = 0
                    Q_S = self.Q_values[current_S][action]
                elif current_S == LOSE_STATE:
                    R_S = -1
                    MaxV = 0
                    Q_S = self.Q_values[current_S][action]
                    ...
                    R_S = 0
                    MaxV = max(self.Q_values[next_S])
                   
                    Q_S = self.Q_values[current_S][action]
                ALPHA = self.alpha(self.N_Q[current_S][action])


                self.Q_values[current_S][action] = Q_S + ALPHA * (R_S + DISCOUNT * MaxV - Q_S)
    ...
    def pick_action(self, s, epsilon):
        ...
        if random.random() < epsilon:
            ...
            max_value = max(self.Q_values[s])
            return self.Q_values[s].index(max_value)
